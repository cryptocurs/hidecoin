...
module.exports = new class Miner extends Component {
  ...
  run(minerAddresses = null) {
    ...
    helper.processListSync(R.keys(storage.freeTxs), {
      onProcess: (item, callback, toReturn) => {
        Tx.isValid(helper.hexToBuf(item), helper.baseToBuf(storage.freeTxs[item].data), null, blockchain.getLength(), false, (valid, err) => {
          if (!valid) {
            Tx.freeTxDelete(item, true)
