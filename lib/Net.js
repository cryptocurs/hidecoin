...
class Net extends Component {
    ...
    this.processCommand = (msg, rinfo, localPartialId) => {
      if (msg[0] === CmdPacker.CMD_SRV_INFO) {
        ...
        if (data && blockchainLength) {
          ...
          this.log('{yellow-fg}REQUEST_BLOCKS_AFTER' + (zipAllowed ? ' +Z' : '') + ' ' + data.count + ' from ' + rinfo.address + '{/yellow-fg}')
          const timeStart = helper.unixTimeMs()
          ...
          if (data.hash.equals(Buffer.from('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 'hex'))) {
            ...
            if (block) {
              blockId = block.id
              maxBlockId = blockId + data.count
              if (maxBlockId > blockchainLength) {
                maxBlockId = blockchainLength
              }
              let blocks = []
              blockchain.eachFromTo(blockId, maxBlockId, (block) => {
                blocks.push({hash: block.hash, data: block.data})
              })
              this.send(CmdPacker.pack(CmdPacker.CMD_TAKE_BLOCKS_AFTER, {flags: [CmdPacker.FLAG_ZIPPED], afterHash: data.hash, afterId: blockId - 1, blocks: blocks}), rinfo.port, rinfo.address)
            }
          } else if (Block.known(data.hash)) {
            const block = blockchain.getWithPrevBlock(data.hash)
            if (block) {
              ...
              let curDataPacket
              blockchain.eachFromTo(blockId, maxBlockId, (block) => {
                const blockData = Buffer.allocUnsafeSlow(block.data.length)
                block.data.copy(blockData)
                blocks.push({hash: block.hash, data: blockData})
                nextDataPacket = CmdPacker.pack(CmdPacker.CMD_TAKE_BLOCKS_AFTER, {flags: [CmdPacker.FLAG_ZIPPED], afterHash: data.hash, afterId: blockId - 1, blocks: blocks})
                if (nextDataPacket.length > 1572864) {
                  return false
                }
                curDataPacket = nextDataPacket
              })
              this.send(curDataPacket, rinfo.port, rinfo.address)
            } else {
              this.send(CmdPacker.pack(CmdPacker.CMD_NO_BLOCK_AFTER, {hash: data.hash}), rinfo.port, rinfo.address)
            }
          } else {
                ...
                onRcvd: (msg) => {
                  if (msg[0] === p2p.PACK_DATA_PART_OK) {
                    ...
                    if (accepted === partsCount) {
                      ...
                      const received = accepted * 20 / partsCount >> 0
                      this.logAliasBy('P2X', partialIdStr, 'MPX ' + partialIdStr + ' ' + _.repeat('|', received) + _.repeat('.', 20 - received))
                    }
                  } else {
                    rejected = true
  ...
  broadcast(data, ignoreBroadcastLog = false, toSend = 50) {
    const sendNext = () => {
      if (!addresses[index]) {
        return false
      }
