      ...
      if (msg[0] === CmdPacker.CMD_SRV_INFO) {
        ...
        if (data) {
          if (Block.known(data.hash)) {
            ...
            this.send(CmdPacker.pack(CmdPacker.CMD_NO_BLOCK_AFTER, {hash: data.hash, len: blockchain.getLength()}), rinfo.port, rinfo.address)
            }
          } else {
            this.send(CmdPacker.pack(CmdPacker.CMD_NO_BLOCK, {hash: data.hash, len: blockchain.getLength()}), rinfo.port, rinfo.address)
          }
        }
        ...
        let data = CmdPacker.unpack(msg)
        data && this.trigger('blockAfterNoBlock', data.hash, data.len)
      } else if (msg[0] === CmdPacker.CMD_NO_BLOCK_AFTER) {
        let data = CmdPacker.unpack(msg)
        data && this.trigger('blockAfterNoBlockAfter', data.hash, data.len)
