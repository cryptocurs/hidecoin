...
class Net extends Component {
    ...
    this.processCommand = (msg, rinfo) => {
      if (msg[0] === CmdPacker.CMD_SRV_INFO) {
        ...
        if (data) {
          ...
          if (blockWithHash) {
            if (blockWithHash.id < blockchain.getLength() - 200) {
              return
            }
            let block
            let prevBlock = data.hash
            let hashes = []
            while (block = Block.getWithBuffer('prevBlock', prevBlock)) {
  ...
  connect(config, disallowServerMode = false) {
    ...
    p2p.on('online', () => {
      ...
      this.currentServer.address && storage.servers[this.currentServer.address] && storage.servers[this.currentServer.address].rating++
    })
