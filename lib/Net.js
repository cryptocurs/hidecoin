...
class Net extends Component {
  ...
  constructor() {
    ...
    this.processCommand = (msg, rinfo) => {
      if (msg[0] === CmdPacker.CMD_SRV_INFO) {
        ...
        if (this.ignoreRequestsOfBlocks || storage.session.synchronizing && helper.randomBool() || storage.session.forkProcessor) {
        ...
        if (this.partialBuffers[localPartialId]) {
          ...
          if (!partialBuffer.partsReceived[partId]) {
            ...
            if (partialBuffer.receivedLength === partialBuffer.dataLength) {
              this.logBy('P2X', 'Multi-part transfer ' + localPartialId + ' received, checksum ' + helper.checksum(partialBuffer.data))
  ...
  send(data, port, address, callbacks) {
    if (data.length > PARTIAL_TRANSFER_SIZE) {
      ...
      p2p.sendWait(port, address, Packet(p2p.PACK_DATA_PART_SIZE).packFixed(p2p.getId()).packFixed(partialId).packNumber(dataLength, 4).packNumber(PARTIAL_TRANSFER_SIZE, 4).get(), p2p.PACK_DATA_PART_SIZE_OK, 2000, 5, {
        onRcvd: (msg) => {
