...
class Net extends Component {
    ...
    this.processCommand = (msg, rinfo, localPartialId) => {
      if (msg[0] === CmdPacker.CMD_SRV_INFO) {
        ...
        if (data && blockchainLength) {
          ...
          this.log('{yellow-fg}REQUEST_BLOCKS_AFTER ' + data.count + ' from ' + rinfo.address + '{/yellow-fg}')
          const timeStart = helper.unixTimeMs()
          ...
          if (data.hash.equals(Buffer.from('FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF', 'hex'))) {
            ...
            if (block) {
              ...
              this.send(CmdPacker.pack(CmdPacker.CMD_TAKE_BLOCKS_AFTER, {afterHash: data.hash, afterId: blockId - 1, blocks: blocks}), rinfo.port, rinfo.address)
            }
          } else if (Block.known(data.hash)) {
            const block = blockchain.getWithPrevBlock(data.hash)
            if (block) {
          ...
          if (blockId) {
            this.log('{green-fg}Blocks #' + blockId + '-' + (maxBlockId - 1) + ' was read in ' + (helper.unixTimeMs() - timeStart) + ' ms{/green-fg}')
          } else {
            this.log('{cyan-fg}Work with blockchain completed in ' + (helper.unixTimeMs() - timeStart) + ' ms{/cyan-fg}')
          }
        }
        ...
        let data = CmdPacker.unpack(msg)
        data && this.trigger('blocksAfterRcvd', data.afterHash, data.blocks, data.blocksCount, localPartialId)
  ...
  connect(disallowServerMode = false) {
    setInterval(() => {
    ...
    !disallowServerMode && p2p.listen(storage.config.myServerPort)
  ...
  broadcast(data, ignoreBroadcastLog = false, toSend = 7) {
    ...
    var addresses = _.shuffle(R.filter(i => (storage.config.allowIpv4 && !storage.servers[i].isIpv6) || (storage.config.allowIpv6 && storage.servers[i].isIpv6), R.keys(storage.servers)))
