...
const CMD_INFO_REQUEST_BLOCKCHAIN_LENGTH = 0x80
const CMD_INFO_TAKE_BLOCKCHAIN_LENGTH = 0x90
const CMD_REQUEST_TIME = 0xe0
const CMD_TAKE_TIME = 0xf0

const FLAG_ZIP_ALLOWED = 0x01
...
const functions = {
  pack: (cmd, data) => {
    ...
    if (cmd === CMD_SRV_INFO) {
      ...
      packet.packNumber(flags, 1).packFixed(data.afterHash).packNumber(data.afterId, 4).packNumber(data.blocks.length, 2)
      const blocksPacket = PacketBig()
      R.forEach((block) => {
        blocksPacket.packFixed(block.hash).packNumber(block.data.length, 4).packFixed(block.data)
      }, data.blocks)
      const blocksBuffer = blocksPacket.get()
      
      if (R.contains(FLAG_ZIPPED, data.flags)) {
        packet.packFixed(zlib.deflateRawSync(blocksBuffer))
      } else {
        packet.packFixed(blocksBuffer)
      }
    ...
    } else if (cmd === CMD_INFO_REQUEST_BLOCKCHAIN_LENGTH) {
    } else if (cmd === CMD_INFO_TAKE_BLOCKCHAIN_LENGTH) {
      packet.packNumber(data.blockchainLength, 4)
  ...
  unpack: (data) => {
    ...
    if (data[0] === CMD_SRV_INFO) {
      ...
      let blocksBuffer
      if (functions.isFlagSet(res.flags, FLAG_ZIPPED)) {
        try {
          blocksBuffer = zlib.inflateRawSync(data.slice(40))
        } catch (e) {
          return false
        }
      } else {
        blocksBuffer = data.slice(40)
      }
      let pos = 0
      res.blocks = []
      for (let i = 0; i < res.blocksCount; i++) {
        if (pos + 36 > blocksBuffer.length) {
          return false
        }
        let block = {}
        block.hash = blocksBuffer.slice(pos, pos += 32)
        block.size = blocksBuffer.readUInt32BE(pos)
        block.data = blocksBuffer.slice(pos += 4, pos += block.size)
    ...
    } else if (data[0] === CMD_INFO_REQUEST_BLOCKCHAIN_LENGTH) {
      if (data.length !== 1) {
        return false
      }
    } else if (data[0] === CMD_INFO_TAKE_BLOCKCHAIN_LENGTH) {
      if (data.length !== 5) {
        return false
      }
      res.blockchainLength = data.readUInt32BE(1)
  ...
  toStr: (cmd) => {
    switch (cmd) {
      ...
      case CMD_INFO_REQUEST_BLOCKCHAIN_LENGTH: return 'CMD_INFO_REQUEST_BLOCKCHAIN_LENGTH'
      case CMD_INFO_TAKE_BLOCKCHAIN_LENGTH: return 'CMD_INFO_TAKE_BLOCKCHAIN_LENGTH'
...
module.exports.CMD_INFO_REQUEST_BLOCKCHAIN_LENGTH = CMD_INFO_REQUEST_BLOCKCHAIN_LENGTH
module.exports.CMD_INFO_TAKE_BLOCKCHAIN_LENGTH = CMD_INFO_TAKE_BLOCKCHAIN_LENGTH
module.exports.CMD_REQUEST_TIME = CMD_REQUEST_TIME
module.exports.CMD_TAKE_TIME = CMD_TAKE_TIME
module.exports.FLAG_ZIP_ALLOWED = FLAG_ZIP_ALLOWED
module.exports.FLAG_ZIPPED = FLAG_ZIPPED