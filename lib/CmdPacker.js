...
module.exports = {
  pack: (cmd, data) => {
    const packet = (R.contains(cmd, [CMD_TAKE_BLOCK_AFTER, CMD_TAKE_HASHES_AFTER, CMD_BLOCK_FOUND, CMD_TX_INFO]) ? PacketBig(cmd) : Packet(cmd))
    if (cmd === CMD_SRV_INFO) {
      ...
      packet.packFixed(data.hash).packNumber(data.id, 4).packNumber(data.count, 1)
      ...
      packet.packFixed(data.afterHash).packNumber(data.afterId, 4).packNumber(data.blocks.length, 1)
      R.forEach((block) => {
        packet.packFixed(block.hash).packNumber(block.length, 4).packFixed(block.data)
      }, data.blocks)
  ...
  unpack: (data) => {
    ...
    if (data[0] === CMD_SRV_INFO) {
      ...
      res.hash = data.slice(1, 33)
      res.id = data.readUInt32BE(33)
      res.count = data.readUInt8(37)
      ...
      res.afterHash = data.slice(1, 33)
      res.afterId = data.readUInt32BE(33)
      res.blocksCount = data.readUInt8(37)
      res.blocks = []
      let pos = 38
