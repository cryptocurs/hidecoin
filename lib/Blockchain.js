...
class Blockchain {
  ...
  constructor() {
    ...
    this.cacheIndex()
    storage.config.blockchainMemory && this.cacheBlocks()
  }
  ...
  add(hash, data, unpacked, callback) {
    if (this.workers) {
      ...
      this.length++
      callback && callback()
      storage.trigger('blockchainAddedBlock', unpacked)
    }
  }
  ...
  get(id) {
    ...
    if (storage.config.blockchainMemory) {
      this.workers--
      return {id: id, hash: hash, data: this.blocksCached.slice(start, start + size)}
    }
