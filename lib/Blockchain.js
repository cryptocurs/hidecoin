...
const MAX_LAST_BLOCKS_IN_CACHE = 10
...
class Blockchain extends Component {
  ...
  constructor() {
    ...
    this.lastBlocksCached = {}
    
    this.cacheIndex = (fdIndex) => {
      this.indexSize = this.length * 40
      this.indexCached = Buffer.allocUnsafe(this.indexSize)
      this.workers++
    ...
    this.cacheBlocks = (fd) => {
      ...
      this.blocksCached = Buffer.allocUnsafe(this.blocksSize)
      this.workers++
      fs.readSync(fd, this.blocksCached, 0, this.blocksSize, 0)
      this.workers--
    }
    
    this.cacheLastBlock = (id, hash, data) => {
      const minId = this.length - MAX_LAST_BLOCKS_IN_CACHE
      if (id > minId) {
        this.lastBlocksCached[id] = {hash, data}
        this.uncacheLastBlocksTo(minId)
      }
    }
    
    this.uncacheLastBlock = (id) => {
      delete this.lastBlocksCached[id]
    }
    
    this.uncacheLastBlocksFrom = (id) => {
      for (const i in this.lastBlocksCached) {
        i >= id && this.uncacheLastBlock(i)
      }
    }
    
    this.uncacheLastBlocksTo = (id) => {
      for (const i in this.lastBlocksCached) {
        i <= id && this.uncacheLastBlock(i)
      }
    }
  ...
  add(hash, data, unpacked, callback) {
    if (this.workers) {
      ...
      if (storage.config.blockchainMemory) {
        const _blocksCached = Buffer.allocUnsafe(this.blocksSize)
  ...
  removeLast(callback, count = 1) {
    ...
    if (this.workers) {
      ...
      this.uncacheLastBlocksFrom(this.length)
      this.workers--
      callback && callback()
  ...
  get(id) {
    ...
    const {hash: lastBlockHash, data} = this.lastBlocksCached[id] || {}
    if (data) {
      this.log('Loaded block #' + id + ' from cache')
      return {id, hash: lastBlockHash, data}
    }
    ...
    this.cacheLastBlock(id, hash, buffer)
    this.workers--
    this.log('Loaded block #' + id + ' from file')
  ...
  getWithPrevBlock(blockHash, onlyId = false) {
    if (blockHash.equals(BUFFER_FF)) {
      return onlyId ? this.length ? 0 : -1 : blockchain.get(0)
    }
    const id = this.getWithHash(blockHash, true)
    if (id === false) {
      return onlyId ? -1 : null
    }
    
    return onlyId ? id < this.length - 1 ? id + 1 : -1 : blockchain.get(id + 1)
  }
  ...
  eachFromTo(minId, maxId, callback, returnDefault = false) {
    ...
    while ((id < maxId) && (pos + 40 <= this.indexSize)) {
      ...
      try {
        ...
        this.log({blocksSize: this.blocksSize, startNext, size, start})
      }
  ...
  addToIndexCached(indexRecord) {
    ...
    this.indexSize += 40
    if (this.indexSize > this.indexCached.length) {
      const _indexCached = Buffer.allocUnsafe(prevIndexSize)
      this.indexCached.copy(_indexCached)
      this.indexCached = Buffer.allocUnsafe(this.indexSize + 20000)
