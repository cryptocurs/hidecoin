...
class Wallet extends Component {
  ...
  sendCoins(data, callback) {
    ...
    */
    ...
    const createTx = ({address, toSend, toReceive}, walletData, callback) => {
      const sortedAddrs = R.sort((a, b) => {
        return walletData.balances[b] - walletData.balances[a]
      }, R.keys(walletData.balances))
      
      let txIns = []
      let txOuts = [{address: Address.hashToRaw(address), value: toReceive}]
      let txKeys = []
      let txKeyId = -1
    ...
    const calcFee = () => {
      this.log('Create tx', {address, toSend, toReceive})
      createTx({address, toSend, toReceive}, walletData, (txPacked, tx) => {
