...
class Wallet extends Component {
  ...
  spendTransaction(data, callback) {
    const {hash, outN, address} = data
    const hashRaw = Buffer.from(hash, 'hex')
    const tx = Tx.get(hashRaw)
    if (!tx) {
      callback({type: 'error', message: 'Transaction does not exist'})
      return
    }
    
    const txOut = Tx.unpack(tx.data).txOuts[outN]
    if (!txOut) {
      callback({type: 'error', message: 'OUT does not exist'})
      return
    }
    
    if (Tx.isOutSpent(hashRaw, outN) || Tx.isOutSpentFreeTxs(hashRaw, outN)) {
      callback({type: 'error', message: 'OUT is spent'})
      return
    }
    ...
    for (const i in this.keys) {
      addr = new Address(this.keys[i])
      if (addr.getAddressRaw().equals(txOut.address)) {
        found = true
        break
      }
    }
    if (!found) {
      callback({type: 'error', message: 'OUT does not belong to this wallet'})
      return
    }
    
    const fee = Tx.MIN_FEE
    const value = txOut.value - fee
    if (value < 0) {
      callback({type: 'error', message: 'Not enough micoins'})
      return
    }
    ...
    const txIns = [{
      ...
      keyId: txKeyId,
      sign: null,
      priv: addr.getKeys().priv
    }]
    const txOuts = [{
      address: Address.hashToRaw(address),
      value
    }]
    ...
    helper.processList(txIns, {
      onProcess: (item, callback) => {
        helper.signData(Buffer.concat([Tx.packHashOutN(item), txOutsRaw]), item.priv, (sign) => {
          item.sign = sign
          delete item.priv
          callback()
        })
      ...
      onReady: () => {
        ...
        const txPacked = Tx.pack(tx)
        const txHash = helper.hash(txPacked)
        
        Tx.isValid(txHash, txPacked, null, blockchain.getLength(), false, (valid, err) => {
          if (valid) {
            Tx.freeTxAdd(txHash, txPacked, fee)
            miner.restart()
            callback({type: 'success', message: 'Coins has been sent with fee ' + fee + ' micoins', amount: value, fee})
