...
const functions = {
  ...
  get: (hash, id = null) => {
    return storage.session.blockchain.txMap.each(({blockId, txHash}) => {
      if (id !== null && blockId >= id) {
        return null
      }
      if (hash.equals(txHash)) {
        const {hash: blockHash, data: blockData} = blockchain.get(blockId)
        const txHashList = BlockHelper.unpackHashList(blockData)
        for (const t in txHashList) {
          if (hash.equals(txHashList[t])) {
            return {blockId, blockHash, data: BlockHelper.unpack(blockData).txList[t]}
          }
        }
      }
    })
  ...
  known: (hash) => {
    return storage.session.blockchain.txMap.each(({txHash}) => {
      if (hash.equals(txHash)) {
        return true
      }
    }, false)
  ...
  isOutSpent: (hash, outN) => {
    const indexes = helper.sortedIndexesOfBA(storage.session.blockchain.spends, hash, helper.bufferCompareBA, 'txInHash')
    for (const i in indexes) {
      const index = indexes[i]
      const {blockId, txInOutN, txHash} = storage.session.blockchain.spends.get(index)
      if (txInOutN === outN) {
        return {blockId, txHash}
      }
    }
    return null
  },
  ...
  isOwn: (txIns, ownAddresses) => {
    if (txIns.length) {
      for (const i in txIns) {
        const {txHash, outN} = txIns[i]
        const {txOuts} = functions.unpack(functions.get(txHash).data)
        if (!R.contains(txOuts[outN].address, ownAddresses)) {
          return false
        }
      }
    } else {
  ...
  calcFee: (txSize) => {
    return txSize * MIN_FEE_PER_BYTE
  },
...
module.exports.MIN_FEE = MIN_FEE
module.exports.MIN_FEE_PER_BYTE = MIN_FEE_PER_BYTE
