...
const functions = {
    ...
    return blockchain.eachTo(id === null ? blockchain.getLength() : id, (block) => {
      ...
      for (let i in txHashList) {
        if (hash.equals(txHashList[i])) {
          return BlockHelper.unpack(block.data).txList[i]
        }
      }
    })
  },
  ...
  isOutSpent: (hash, outN, id = null) => {
    return blockchain.eachTo(id === null ? blockchain.getLength() : id, (block) => {
      let blockUnpacked = BlockHelper.unpack(block.data)
      var toReturnTrue = false
      R.forEach((tx) => {
        let txUnpacked = functions.unpack(tx)
        R.forEach((txIn) => {
          if (txIn.txHash.equals(hash) && (txIn.outN === outN)) {
						toReturnTrue = true
            return
					}
        }, txUnpacked.txIns)
      ...
      }, blockUnpacked.txList)
      if (toReturnTrue) {
        return true
      }
    })
  },
  isOutSpentFreeTxs: (hash, outN, id = null) => {
  ...
  isValid: (hash, tx, blockInfo, id, isFirstBlockTx, callback, notFirstBlockTxsFee = 0, unpacked = null) => {
    ...
    let txOutSum = 0
    let toVerify = []
    for (let i in txUnpacked.txIns) {
      i = parseInt(i)
      let txIn = txUnpacked.txIns[i]
      let txWithOut = functions.get(txIn.txHash, id)
        ...
        for (let t in blockOtherTxs) {
          ...
          for (let x in txData.txIns) {
            ...
            if ((txIn.txHash.equals(blockTxIn.txHash))
              ...
              if (blockCurTxId !== t) {
                lastValidationError = 'Double spend with other txs in block'
                callback(false)
                return
              }
            }
