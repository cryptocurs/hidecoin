...
const functions = {
  ...
  isValid: (hash, tx, blockInfo, id, isFirstBlockTx, callback, notFirstBlockTxsFee = 0, unpacked = null) => {
    ...
    for (let i in txUnpacked.txIns) {
      ...
      let txWithOut = txWithOutInfo.data
      if ((txWithOutInfo.blockId >= 11000) && (id - txWithOutInfo.blockId < MIN_CONFIRMATIONS)) {
        lastValidationError = 'Tx in IN ' + i + ' do not have ' + MIN_CONFIRMATIONS + ' confirmations'
        callback(false)
        return
      }
