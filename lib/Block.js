      ...
      for (let i in blockUnpacked.txList) {
        ...
        for (let t in txUnpacked.txOuts) {
          ...
          if (txOut.address.equals(address)) {
            ...
            let txWithOutSpentFreeTxs = Tx.isOutSpentFreeTxs(txHash, t)
            txs.push({blockId: block.id, blockTime: blockUnpacked.time, hash: txHash, outN: t, value: txOut.value, spent: txWithOutSpent, spentFreeTxs: txWithOutSpentFreeTxs})
      ...
      for (let i in blockUnpacked.txList) {
        ...
        for (let t in txUnpacked.txOuts) {
          ...
          if (R.contains(txOut.address, addresses)) {
            let txWithOutSpent = Tx.isOutSpent(txHash, t)
            let txWithOutSpentFreeTxs = Tx.isOutSpentFreeTxs(txHash, t)
            txs.push({blockId: block.id, blockTime: blockUnpacked.time, hash: txHash, outN: t, address: txOut.address, value: txOut.value, spent: txWithOutSpent, spentFreeTxs: txWithOutSpentFreeTxs})
            if (!txWithOutSpent && !txWithOutSpentFreeTxs) {
              balance += txOut.value
            }
          }
