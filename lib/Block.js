      ...
      for (let i in blockUnpacked.txList) {
        ...
        for (let t in txUnpacked.txOuts) {
          ...
          if (txOut.address.equals(address)) {
            txs.push({hash: txHash, outN: t, value: txOut.value, spent: txWithOutSpent})
            if (!txWithOutSpent && !Tx.isOutSpentFreeTxs(txHash, t)) {
              balance += txOut.value
            } else if (txWithOutSpent) {
              outTxs.push(txWithOutSpent)
            }
          }
        }
      }
    })
    return {balance: balance, txs: txs, outTxs: outTxs}
  },
