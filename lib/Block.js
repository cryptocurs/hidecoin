...
const functions = {
    ...
    blockchain.eachTo(id === null ? blockchain.getLength() : id, (block) => {
      ...
      for (let i in blockUnpacked.txList) {
        ...
        for (let t in txUnpacked.txOuts) {
            ...
            if (txOut.address.equals(address) && !Tx.isOutSpent(txHash, t, id) && !Tx.isOutSpentFreeTxs(txHash, t, id)) {
              balance += txOut.value
              txs.push({hash: txHash, outN: t, value: txOut.value})
            }
        }
      }
    })
  ...
  known: (hash) => {
    return (functions.getWithHash(hash) !== false)
  },
