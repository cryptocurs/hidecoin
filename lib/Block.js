...
const functions = {
  ...
  pack: (data) => {
    const packet = Packet().packNumber(data.ver, 1).packFixed(data.prevBlock).packNumber64(data.time).packFixed(data.diff).packNumber64(data.nonce)
      ...
      for (let i in blockUnpacked.txList) {
        ...
        for (let t in txUnpacked.txOuts) {
          ...
          if (txOut.address.equals(address)) {
            let txWithOutSpent = Tx.isOutSpent(txHash, t)
            let txWithOutSpentFreeTxs = Tx.isOutSpentFreeTxs(txHash, t)
            let confirmations = maxId - block.id
  ...
  isValidExisting: (id, hash, packed, callback, prepared = null) => {
    ...
    // hash
    let calcedHash = helper.hash(unpacked.ver === 1 ? packed : unpacked.headerRaw)
