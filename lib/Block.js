...
const functions = {
  ...
  getAddressesBalanceSepOld: (addresses, id = null) => {
    const maxId = (id === null ? blockchain.getLength() : id)
    let result = {}
      ...
      for (let i in blockUnpacked.txList) {
        ...
        for (let t in txUnpacked.txOuts) {
          ...
          if (R.contains(txOut.address, rawAddresses)) {
            ...
            const confirmations = maxId - block.id
            if (confirmations < Tx.MIN_CONFIRMATIONS) {
              result[addressHashed].txsSoft.push({blockId: block.id, blockTime: blockUnpacked.time, hash: txHash, outN: t, value: txOut.value, spent: txWithOutSpent, spentFreeTxs: txWithOutSpentFreeTxs, confirmations: confirmations})
            } else {
              result[addressHashed].txsHard.push({blockId: block.id, blockTime: blockUnpacked.time, hash: txHash, outN: t, value: txOut.value, spent: txWithOutSpent, spentFreeTxs: txWithOutSpentFreeTxs, confirmations: confirmations})
            }
            if (!txWithOutSpent && !txWithOutSpentFreeTxs) {
              if (confirmations < Tx.MIN_CONFIRMATIONS) {
                result[addressHashed].balanceSoft += txOut.value
              } else {
                result[addressHashed].balanceHard += txOut.value
              }
            }
          }
        }
  ...
  getAddressesBalanceSep: (addresses, id = null) => {
    const maxId = (id === null ? blockchain.getLength() : id)
    let result = {}
    for (let i in addresses) {
      const addr = addresses[i]
    ...
    blockchain.eachTo(maxId, (block) => {
      ...
      for (let i in blockUnpacked.txList) {
        ...
        for (let x in txUnpacked.txIns) {
          let isSpent = false
          const txIn = txUnpacked.txIns[x]
          for (let r in result) {
            const resultAddr = result[r]
            for (let h in resultAddr.txsSoft) {
              const txWithOut = resultAddr.txsSoft[h]
              if (txIn.txHash.equals(txWithOut.hash) && (txIn.outN === txWithOut.outN)) {
                resultAddr.balanceSoft -= txWithOut.value
                txWithOut.spent = true
                isSpent = true
                break
              }
            }
            ...
            for (let h in resultAddr.txsHard) {
              const txWithOut = resultAddr.txsHard[h]
              if (txIn.txHash.equals(txWithOut.hash) && (txIn.outN === txWithOut.outN)) {
                resultAddr.balanceHard -= txWithOut.value
                txWithOut.spent = true
                isSpent = true
                break
              }
        ...
        for (let t in txUnpacked.txOuts) {
          ...
          if (R.contains(txOut.address, rawAddresses)) {
            ...
            const resultAddr = result[addressHashed]
            const confirmations = maxId - block.id
            const isSoft = (confirmations < Tx.MIN_CONFIRMATIONS)
            
            const txWithOutSpentFreeTxs = Tx.isOutSpentFreeTxs(txHash, t)
            if (isSoft) {
              resultAddr.txsSoft.push({blockId: block.id, blockTime: blockUnpacked.time, hash: txHash, outN: t, value: txOut.value, spent: false, spentFreeTxs: txWithOutSpentFreeTxs, confirmations: confirmations})
            } else {
              resultAddr.txsHard.push({blockId: block.id, blockTime: blockUnpacked.time, hash: txHash, outN: t, value: txOut.value, spent: false, spentFreeTxs: txWithOutSpentFreeTxs, confirmations: confirmations})
            }
            if (!txWithOutSpentFreeTxs) {
              if (isSoft) {
                resultAddr.balanceSoft += txOut.value
              } else {
                resultAddr.balanceHard += txOut.value
              }
            }
          }
        }
