...
const functions = {
  ...
  getAddressesBalanceSep: (addresses, id = null, callbacks) => {
    const maxId = (id === null ? blockchain.getLength() : id)
    let result = {}
    ...
    if (!storage.localSession.wcache) {
      storage.localSession.wcache = WCache(storage.session.login)
    }
    
    let minId = 0
    const {blockId, data} = storage.localSession.wcache.get('checkpoint') || {}
    if (blockId) {
      minId = blockId + 1
      // copy each address because new addresses might be created
      for (const addr in data) {
        result[addr] = data[addr]
      }
      callbacks && callbacks.onCheckpointLoaded && callbacks.onCheckpointLoaded(blockId)
    }
    ...
    blockchain.eachFromTo(minId, maxId, (block) => {
      ...
      if (block.id === blockchainLength - 1000 && block.id > minId + 500) {
        storage.localSession.wcache.set('checkpoint', {blockId: block.id, data: result})
        callbacks && callbacks.onCheckpointCreated && callbacks.onCheckpointCreated(block.id)
      }
    })
    return result
  },
