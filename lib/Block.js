...
const functions = {
  ...
  getAddressesBalanceSep: (addresses, id = null, callbacks = {}, login = null) => {
    ...
    const processTx = ({addressHashed, blockId, blockTime, hash, outN, value}) => {
      ...
      for (let i in blockUnpacked.txList) {
        ...
        for (let t in txUnpacked.txOuts) {
          ...
          if (R.contains(txOut.address, rawAddresses)) {
            ...
            processTx({addressHashed, blockId: block.id, blockTime: blockUnpacked.time, hash: txHash, outN: t, value: txOut.value})
          }
        }
      }
