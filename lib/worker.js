...
process.on('message', ({cmd, cmdData}) => {
  ...
  if (cmd === 'update.blockchain.spends') {
    ...
    if (clusterName === 'block') {
      ...
      // TODO: automatic update of cache (spends) from master
      // TODO: automatic update of blockchain index from master
      // TODO: automatic update of blockchain for blockchainMemory=true or remove blockchainMemory
      if (!synchronizer.isCached()) {
        synchronizer.cache()
        send({state: 'blockchain.cache.spends.created'})
      }
    }
    if (kind === 'block.cache') {
      send({result: {spends: storage.session.blockchain.spends}})
