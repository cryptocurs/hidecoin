...
process.on('message', ({cmd, cmdData}) => {
  ...
  if (cmd === 'update.blockchain.spends') {
    ...
    const [blockId, txHash] = cmdData.txInfo
    synchronizer.addTxToMap(blockId, txHash)
    ...
    if (kind === 'block.cache') {
      ...
      Block.isValidNew(params.hash, params.data, (valid, err, info) => {
        const {unpacked, txUnpackedList} = info || {}
        send({state: 'block.validating.done=' + (valid ? 'valid' : 'not valid ' + err + ' ' + blockchain.getLength())})
        send({result: {valid, unpacked, txUnpackedList, lastBlockValidationError: err}})
      }, params.promiscuous)
