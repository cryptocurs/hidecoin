...
if (config.minerMinimalFee >= 1000000) {
  console.log('\x1b[31mconfig.minerMinimalFee is too big. Recommended value is 10000.\x1b[0m')
}
if (config.singleThreaded === false) {
  console.log('\x1b[31mRecommended value of config.singleThreaded is true.\x1b[0m')
}
...
if (!storage.config.minerMinimalFeePerByte) {
  storage.config.minerMinimalFeePerByte = 30
  console.log('\x1b[33mconfig.minerMinimalFeePerByte set to default: ' + storage.config.minerMinimalFeePerByte + '\x1b[0m')
}
if (storage.config.singleThreaded === undefined) {
  storage.config.singleThreaded = true
  console.log('\x1b[33mconfig.singleThreaded set to default: true\x1b[0m')
}
...
storage.logIgnoreModules = {P2P: true, THR: true, BLK: true}
...
if (storage.config.singleThreaded) {
  ...
  storage.on('ready', () => {
    setTimeout(() => {
      !synchronizer.isCached() && storage.trigger('logAlias', 'blockchainCaching', 'Caching blockchain (this may take a few minutes)...')
    }, 2000)
  })
  dispTask.blockCache((err, res) => {
    storage.session.blockchain.spends = res.spends
    storage.session.blockchain.txMap = helper.unzipTxMap(res.txMap)
    storage.trigger('logAliasClear', 'blockchainCaching')
    log('CORE: Blockchain is cached. Spendings: ' + storage.session.blockchain.spends.length + ', txs in map: ' + storage.session.blockchain.txMap.length)
  })
}
...
app.post('/', (req, res) => {
  ...
  updateData((updated) => {
    if (updated) {
      ...
      let errors = ''
      if (config.minerMinimalFee >= 1000000) {
        errors += '<div class="alert alert-danger">config.minerMinimalFee is too big. Recommended value is 10000. You will be charged an extra fee if you do not change this setting.</div>'
      }
