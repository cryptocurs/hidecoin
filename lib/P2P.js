...
module.exports = new class P2P extends Component {
    ...
    this.processMessage = (msg, rinfo) => {
      ...
      if (msg[0] === PACK_PING) {
        ...
        if (id.equals(this.uniqueId)) {
          ...
          this.send(Packet(PACK_PING).get(), port, rinfo.address, rinfo.family)
          this.wait(PACK_PONG, port, rinfo.address, 2000, {
            onRcvd: () => {
        ...
        this.clientTimer = setInterval(() => {
          this.send(Packet(PACK_PING).get(), this.server[1], this.server[0])
          this.wait(PACK_PONG, this.server[1], this.server[0], 2000, {
            onTimeout: () => {
  ...
  sendAsClient(data, port, address, family = null) {
    if (this.sockets.client) {
      ...
      this.sockets.client.send(data, port, address, (err) => {
        if (err) {
          this.trigger('sendError')
        }
      })
