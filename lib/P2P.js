    ...
    this.processMessage = (msg, rinfo) => {
      ...
      if (msg[0] === PACK_PING) {
        ...
        if (id.equals(this.uniqueId)) {
          ...
          this.sendWait(port, rinfo.address, Packet(p2p.PACK_PING).get(), p2p.PACK_PONG, 1000, 3, {
            onRcvd: () => {
    ...
    this.wait(PACK_ANY, port, address, 4000, {
      onRcvd: (msg) => {
        if (msg[0] === PACK_EXT_IP_OK) {
          ...
          if (!disallowServerMode && (this.serverModeAttempts < 3)) {
          ...
          this.clientTimer = setInterval(() => {
            this.sendWait(this.server[1], this.server[0], Packet(p2p.PACK_PING).get(), p2p.PACK_PONG, 1000, 3, {
              onTimeout: () => {
