...
module.exports = new class P2P extends Component {
  ...
  constructor() {
    ...
    this.validateLength = (msg) => {
      return msg.length && (R.contains(msg[0],
        ...
        || (msg[0] === PACK_DATA_PART_REJECT) && (msg.length === 5))
    }
    ...
    this.processMessage = (msg, rinfo) => {
      ...
      if (msg[0] === PACK_PING) {
        ...
        if (id.equals(this.uniqueId)) {
          ...
          this.sendWait(port, rinfo.address, Packet(PACK_PING).get(), PACK_PONG, 1000, 3, {
            onRcvd: () => {
    ...
    this.onServerOnline = () => {
      setInterval(() => {
        for (const address in storage.servers) {
          ...
          this.send(Packet(PACK_CONNECT).packFixed(this.uniqueId).packNumber(this.localServerPort, 2).get(), port, address)
        }
      }, 120000)
    }
  ...
  clientProcessMessage(...data) {
    this.processMessage(...data)
  }
  
  serverProcessMessage(...data) {
  ...
  listen(localServerPort, listenAnyway = false, callback) {
    if (listenAnyway || !this.localServerPort) {
      ...
      this.sockets.server.bind(localServerPort, callback)
    }
  }
  ...
  connect(address, port, disallowServerMode = false) {
    ...
    this.wait(PACK_ANY, port, address, 5000, {
      onRcvd: (msg) => {
        if (msg[0] === PACK_EXT_IP_OK) {
          ...
          if (!disallowServerMode && (this.serverModeAttempts < 5)) {
          ...
          this.clientTimer = setInterval(() => {
            this.sendWait(this.server[1], this.server[0], Packet(PACK_PING).get(), PACK_PONG, 1000, 2, {
              onTimeout: () => {
                clearInterval(this.clientTimer)
                this.clientTimer = null
                !this.serverOnline && this.trigger('offline')
              }
            })
          }, 10000)
  ...
  wait(type, port, address, timeout, callbacks) {
    var multi = type instanceof Array
    var waiterId = (multi ? PACK_ANY : type) + ':' + address + ':' + port
    var subId = 0
    if (this.waiters[waiterId]) {
...
module.exports.PACK_DATA_PART_REJECT = PACK_DATA_PART_REJECT