  ...
  constructor() {
    ...
    this.remoteInternal = () => {
      ...
      if (storage.session.isPackSynchronizing) {
        storage.session.isPackSynchronizing = false
        net.broadcast(CmdPacker.pack(CmdPacker.CMD_REQUEST_BLOCKS_AFTER, {hash: lastHash, count: 256}), true, storage.session.syncSpeed)
      } else {
    ...
    net.on('multipartRcvdFirst', (multipartId, data, callback) => {
      this.log('{yellow-fg}Multi-part transfer for ' + CmdPacker.toStr(data[0]) + ' requested{/yellow-fg}')
      ...
      if (this.rejectNewBlocks && isNewBlocks) {
        this.log('{red-fg}Multi-part transfer rejected: RECEIVING{/red-fg}')
        callback(false)
        return
      }
      ...
      if (isNewBlocks) {
        ...
        this.log('{cyan-fg}Waiting for multi-part ' + multipartId + '{/cyan-fg}')
      }
      ...
      helper.processList(blocks, {
        onProcess: (item, callback, toReturn, i) => {
          ...
          this.add(item.hash, item.data, {
            onKnown: () => {
              this.logAlias('blocksAfter', 'Validated block ' + i + '. KNOWN')
              callback()
            },
            onAccept: (unpacked) => {
              this.trigger('blockAfterAccept', afterHash, item.hash, item.data, unpacked)
              this.logAlias('blocksAfter', 'Validated block ' + i + '. ACCEPT')
              callback()
            },
            onReject: () => {
              this.logAlias('blocksAfter', 'Validated block ' + i + '. REJECT')
              toReturn()
            }
          })
        },
