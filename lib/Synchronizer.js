...
class Synchronizer extends Component {
  ...
  constructor() {
    ...
    if (disp.isMaster()) {
      ...
      setInterval(() => {
        this.requestNetInfoBlockchainLength()
        setTimeout(() => {
          if ((blockchain.getLength() < storage.session.netInfoBlockchainLength || this.lastScheduled < helper.unixTime() - 300) && this.firstSynchronized && !this.blockSyncTimer && !this.blockCheckTimer && !storage.session.synchronizing && !storage.session.forkProcessor) {
    ...
    this.remoteInternal = () => {
      if (!disp.isMaster() || this.blockSyncTimer || this.blockCheckTimer || storage.session.validating || storage.session.forkProcessor) {
        return true
      }
      this.rejectNewBlocks = false
      this.waitForMultipartId = null
      ...
      if (storage.session.isPackSynchronizing) {
        ...
        storage.session.isPackSynchronizing = false
        net.broadcast(CmdPacker.pack(CmdPacker.CMD_REQUEST_BLOCKS_AFTER, {hash: lastHash, count: 128}), true, storage.session.syncSpeed)
      } else {
    ...
    net.on('blocksAfterRcvd', (afterHash, blocks, blocksCount, multipartId) => {
      if (!this.waitForMultipartId && !this.blockSyncTimer) {
        return
      }
    ...
    net.on('netInfoBlockchainLength', (length, address) => {
      if (length >= blockchain.getLength()) {
        ...
        // this.log('Probable blockchain length', storage.session.netInfoBlockchainLength)
      }
    })
    ...
    blockchain.each((block) => {
      ...
      for (let i in blockUnpacked.txList) {
        ...
        for (let t in txUnpacked.txIns) {
          ...
          this.addSpend([block.id, txIn.txHash, txIn.outN, blockUnpacked.txHashList[i]])
        }
      }
    })
  }
  ...
  broadcastTx(hash, tx) {
    net.broadcast(CmdPacker.pack(CmdPacker.CMD_TX_INFO, {hash: hash, tx: tx}), true, 50)
  }
  ...
  addSpend(spend) {
    const pos = helper.sortedIndex(storage.session.blockchain.spends, spend, helper.bufferCompare)
    helper.insertIntoArray(storage.session.blockchain.spends, pos, spend)
    storage.trigger('blockchainSpendsUpdatedAdded', spend)
  }
  
  requestNetInfoBlockchainLength() {
    net.broadcast(CmdPacker.pack(CmdPacker.CMD_INFO_REQUEST_BLOCKCHAIN_LENGTH), true, 50)
  }
}
