  ...
  constructor() {
    ...
    if (disp.isMaster()) {
      ...
      setInterval(() => {
        this.requestNetInfoBlockchainLength()
        setTimeout(() => {
          if (blockchain.getLength() < storage.session.netInfoBlockchainLength && this.firstSynchronized && !this.blockSyncTimer && !this.blockCheckTimer && !storage.session.synchronizing && !storage.session.forkProcessor) {
