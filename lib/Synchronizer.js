...
class Synchronizer extends Component {
  ...
  constructor() {
    ...
    setInterval(() => {
      this.requestNetInfoBlockchainLength()
      setTimeout(() => {
        if (this.lastBlockAcceptedAt < helper.unixTime() - 90 || (blockchain.getLength() < storage.session.netInfoBlockchainLength || this.lastScheduled < helper.unixTime() - 60) && this.firstSynchronized && !this.blockSyncTimer && !this.blockCheckTimer && !storage.session.synchronizing && !storage.session.forkProcessor) {
    ...
    net.on('blockAfterNoBlock', (hash, len, lastBlockHash, port, address) => {
      ...
      if (len >= blockchain.getLength()) {
        ...
        blockchain.removeLast(() => {
          this.clearBlockSyncTimer()
          this.remoteInternal()
        }, Math.min(blockchain.getLength(), 64))
      }
    })
