...
module.exports = new class helper {
  ...
  sortedIndexBA(array, value, compare, field) {
    let low = 0
  ...
  sortedIndexOfBA(array, value, compare, field) {
    const index = this.sortedIndexBA(array, value, compare, field)
  ...
  sortedIndexesOfBA(array, value, compare, field) {
    let indexes = []
    const low = this.sortedIndexOfBA(array, value, compare, field)
    if (low >= 0) {
      indexes.push(low)
