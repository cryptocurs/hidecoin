...
module.exports.isValid = (address) => {
  try {
    var decoded = address instanceof Buffer ? address : Buffer.from(bs58.decode(address))
    var basic = decoded.slice(0, 21)
    var checksum = decoded.slice(21)
    var basicChecksum = crypto.createHash('sha256').update(basic).digest().slice(0, 4)
