...
class Interface {
  ...
  constructor() {
    ...
    storage.on('stat', (data) => {
      ...
      if (data.net) {
        this.stat.net = data.net
      }
      if (data.netRole) {
        this.stat.netRole = data.netRole
      }
      this.boxes.header.setLine(0, '{bold}HPS ' + _.padStart(this.stat.hps, 4) + ' RPS ' + _.padStart(this.stat.rps, 4) + ' BLK ' + _.padStart(this.stat.blk, 8) + ' {' + this.stat.sncColor + '-fg}SNC ' + this.stat.snc + '{/' + this.stat.sncColor + '-fg} ' + _.padStart(this.stat.net, 7) + ' ' + _.padStart(this.stat.netRole, 7) + '{/bold}')
      this.screen.render()
    })
  }
