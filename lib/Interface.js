...
class Interface {
  ...
  constructor() {
    ...
    this.boxes = {
      ...
      footer: blessed.box({
        ...
        right: 0,
        bottom: 0,
        content: 'F1 Cnsl F2 Blks F3 Minr        F5 Snc- F6 Snc+ F7 Prms                 F10 Quit',
        tags: true,
        style: {
          fg: 'white',
          bg: 'blue',
    ...
    if (extraLines > 0) {
      for (let i = 0; i < extraLines; i++) {
        ...
        for (let alias in this.aliases) {
          if (this.aliases[alias].line > 0) {
            this.aliases[alias].line--
          } else {
  ...
  logConsoleAlias(alias, data) {
    if (this.aliases[alias]) {
      this.boxes.console.setLine(this.aliases[alias].line, data)
    } else {
      this.aliases[alias] = {
        line: this.boxes.console.getScreenLines().length + 1
      }
