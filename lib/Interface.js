...
class Interface {
  ...
  open() {
    ...
    this.boxes = {
      ...
      footer: blessed.box({
        ...
        right: 0,
        bottom: 0,
        content: 'F1 Cnsl F2 Blks F3 Minr F4 Wlt                                          F10 Quit',
        tags: true,
        style: {
          fg: 'white',
          bg: 'blue',
    ...
    setInterval(() => {
      ...
      if (this.headerType === 1) {
        ...
        this.boxes.header.setLine(0, '{bold}HPS ' + _.padStart(storage.session.stat.hps, 4)
          ...
          + ' MNR ' + this.minerStates[this.minerState] + _.padStart(storage.session.version, 11) + '{/bold}')
    ...
    this.screen.key('f5', () => {
      this.headerType = this.headerType < 1 ? this.headerType + 1 : 0
    })
    ...
    this.screen.key('C-v', () => {
      storage.trigger('logLastValidationErrors')
    })
  ...
  close() {
    this.screen && this.screen.destroy()
  }
